 ...
 <main class="flex-1 md:ml-64 px-4 transition-all">
  ...
  <div class="w-full mt-3 md:mt-0">
        <p>
          ...
          <span id="totalCount">0</span> users
        </p>
</div>
...
<table id="shippingManifestTable" class="table-x-auto w-full text-sm text-left text-gray-700 border border-gray-200 rounded-lg overflow-hidden shadow-sm">
    <thead class="bg-gray-700 text-white uppercase text-xs font-semibold border-b border-gray-300">
        <tr>
            ...
            <th class="py-3 px-4 min-w-[160px]">Email</th>
            <th class="py-3 px-4 min-w-[180px]">Contact</th>
            <th class="py-3 px-4 min-w-[160px]">Role</th>
    ...
    <tbody class="divide-y divide-gray-200">
        ...
        <td class="py-2 px-4"><?= htmlspecialchars($manifest['full_name']) ?></td>
            <td class="py-2 px-4"><?= htmlspecialchars($manifest['email']) ?></td>
            <td class="py-2 px-4"><?= htmlspecialchars($manifest['phone']) ?></td>
            <td class="py-2 px-4"><?= htmlspecialchars($manifest['role']) ?></td>
            <td class="py-2 flex gap-1 px-4 space-x-1 no-print">
                ...
                <button class="hover:cursor-pointer bg-green-500 text-white p-2 rounded hover:bg-green-600 transition view-btn"
                    data-id="<?= $manifest['employee_id']; ?>">
                    <i class="fa fa-eye" aria-hidden="true"></i>
                </button>

                <!-- <a href="fpdf/shipping-invoice.php?id=<?= $manifest['employee_id'] ?>" target="_blank"
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (importBtn && fileInput) {
    ...
    fileInput.addEventListener('change', () => {
      ...
      fetch('./functions/employee/import-users.php', {
        method: 'POST',
        body: formData
      })
      .then(res => res.text())
