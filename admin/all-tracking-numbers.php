...
$query = "SELECT * FROM tracking_numbers ORDER BY tracking_id ASC";
...
<main class="flex-1 md:ml-64 px-4 transition-all">
    ...
    <div class="w-full flex justify-between items-center flex-col lg:flex-row mt-4 mb-4">
      <div class="w-full mt-3 md:mt-0">
        <p>
          ...
          <span id="totalCount">0</span> tracking numbers
        </p>
      </div>
    ...
    <div class="print-area overflow-x-auto mt-6">
      ...
      <table id="shippingManifestTable" class="table-x-auto w-full text-sm text-left text-gray-700 border border-gray-200 rounded-lg overflow-hidden shadow-sm">
        <thead class="bg-gray-700 text-white uppercase text-xs font-semibold border-b border-gray-300">
          <tr>
			  <th class="py-3 px-4 min-w-[160px]">Tracking ID</th>
            <th class="py-3 px-4 min-w-[160px]">Tracking Number</th>
            <th class="py-3 px-4 min-w-[120px]">Date Received</th>
        ...
        <tbody class="divide-y divide-gray-200">
          ...
          <?php else: ?>
            <?php foreach ($numbers as $manifest): ?>
              <tr class="even:bg-gray-50 hover:bg-gray-100 transition duration-200">
                <td class="py-2 px-4"><?= htmlspecialchars($manifest['tracking_id']) ?></td>
                <td class="py-2 px-4"><?= htmlspecialchars($manifest['tracking_number']) ?></td>
                <td class="py-2 px-4"><?= htmlspecialchars($manifest['date_received']) ?></td>
  ...
  <div id="editModal" class="fixed inset-0 hidden z-50 bg-gray-400/10 backdrop-blur-md bg-opacity-20 flex items-center justify-center">
    <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-2xl relative max-h-screen overflow-y-auto">
      ...
      <h2 class="text-xl font-bold mb-4">Tracking Number Update</h2>
      <form id="updateTrackingNumberForm" method="post">
        <div class="grid grid-cols-1 gap-5">
          <div>
            <label for="trackingNumber" class="block text-gray-700">Tracking Number</label>
            <input type="text" id="trackingNumber" name="trackingNumber" class="bg-gray-100 w-full p-2 border border-gray-300 rounded" placeholder="1YZ12345W6789012345">
          </div>
          <div>
            <label for="dateReceived" class="block text-gray-700">Date Received</label>
            <input type="date" id="dateReceived" name="dateReceived" class="bg-gray-100 w-full p-2 border border-gray-300 rounded" placeholder="">
          </div>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const updateForm = document.getElementById('updateTrackingNumberForm');
  ...
  table.addEventListener('click', async (ev) => {
    ...
    if (editBtn) {
      try {
        ...
        Swal.fire('Error', 'Could not load TrackingNumber details for editing.', 'error');
      }
      return;
    }
    ...
    if (deleteBtn) {
      const TrackingNumberID = deleteBtn.dataset.id;
      if (!TrackingNumberID) return;
      const rowEl = deleteBtn.closest('tr');

      Swal.fire({
        title: 'Are you sure?',
        text: 'This TrackingNumber will be permanently deleted.',
        icon: 'warning',
        ...
        try {
          const res = await fetch('./functions/shipment/delete-tracking-number.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: 'id=' + encodeURIComponent(TrackingNumberID)
          });
          const json = await res.json();
          if (json.success) {
            const idx = originalRows.findIndex(r => r.id === TrackingNumberID);
            if (idx !== -1) originalRows.splice(idx, 1);
            const fidx = filteredRows.findIndex(r => r.id === TrackingNumberID);
            if (fidx !== -1) filteredRows.splice(fidx, 1);

            if (rowEl) rowEl.remove();

            Swal.fire('Deleted!', 'The tracking number has been deleted.', 'success');
            currentPage = 1;
            renderTable();
          } else {
            Swal.fire('Error!', json.message || 'Failed to delete TrackingNumber.', 'error');
          }
        } catch (err) {
          console.error(err);
  ...
  function ensureHiddenIdField(form, idValue) {
    let hidden = form.querySelector('input[name="id"], input#trackingNumber_id');
    if (!hidden) {
      hidden = document.createElement('input');
      hidden.type = 'hidden';
      hidden.name = 'id';
      hidden.id = 'trackingNumber_id';
      form.appendChild(hidden);
    }
    hidden.value = idValue;
  }

  window.openEditModal = (trackingNumber = {}) => {
    if (!editModal) return;

    editModal.classList.remove('hidden');
    document.getElementById('trackingNumber').value = trackingNumber.tracking_number || '';
    document.getElementById('dateReceived').value = trackingNumber.date_received || '';
   
    // ensure we pass the correct primary key to PHP update endpoint
    const idToSet = trackingNumber.tracking_id || trackingNumber.id || '';
    ensureHiddenIdField(updateForm, idToSet);

    const updateSubmitBtn = document.getElementById('submitBtn');
    if (updateSubmitBtn) updateSubmitBtn.textContent = 'Update Tracking Number';
  };
  ...
  if (updateForm) {
    updateForm.addEventListener('submit', async (e) => {
      ...
      try {
        const res = await fetch('./functions/shipment/update-tracking-number.php', {
          method: 'POST',
          body: formData,
          credentials: 'include'
        });
        ...
        if (result.success) {
          Swal.fire({
            icon: 'success',
            text: 'TrackingNumber updated successfully',
            timer: 2500,
          ...
          Swal.fire({
            ...
            html: result.message || 'Failed to update TrackingNumber.'
          });
        }
      } catch (err) {
        Swal.fire({
  ...
  window.printTable = function () {
    ...
    win.document.write('<h3 style="text-align:center;margin:5px 0;">Benjamin Cargo Logistics - Tracking Numbers</h3>');
  ...
  if (downloadCSVBtn) {
    downloadCSVBtn.addEventListener('click', () => {
      ...
      link.setAttribute("download", "TrackingNumbers.csv");
  ...
  if (importBtn && fileInput) {
    ...
    fileInput.addEventListener('change', () => {
      ...
      fetch('./functions/shipment/import-tracking-number.php', {
        method: 'POST',
        body: formData
      })
      .then(res => res.text())
