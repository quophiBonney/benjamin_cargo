<?php 
session_start();
$userID = $_SESSION['employee_id'] ?? null;
if (!$userID) {
    echo json_encode(['success' => false, 'errors' => ['Unauthorized request. Please log in.']]);
    exit;
}

include_once 'includes/auth.php';
$allowed_roles = ['admin', 'manager', 'hr'];
$session_role = strtolower(trim($_SESSION['role'] ?? ''));
if (!in_array($_SESSION['role'] ?? '', $allowed_roles)) {
    header("Location: staffs-dashboard.php");
    exit;
}
?>
<?php include_once 'includes/sidebar.php'; ?>
<?php include_once 'includes/header.php'; ?>
<?php include_once 'includes/app-bar.php';?>
 <main class="flex-1 md:ml-64 px-4 transition-all">
    <?php include_once 'expense-card.php'?>
<div class="bg-white shadow-md rounded-md p-6 mt-5">
    <h3 class="text-2xl font-semibold mb-4">Post Expense</h3>
    <?php if (!empty($errors)): ?>
    <div class="mb-4 p-4 bg-red-100 border border-red-300 text-red-700 rounded">
      <ul class="list-disc list-inside">
        <?php foreach ($errors as $error): ?>
          <li><?= htmlspecialchars($error) ?></li>
        <?php endforeach; ?>
      </ul>
    </div>
    <?php endif; ?>

    <!-- <form method="post" action="add-room.php" enctype="multipart/form-data"> -->
    <form id="addExpenseForm">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div class="mb-3">
        <label for="title" class="block text-gray-700">Title</label>
         <input type="text" id="title" name="title" placeholder="Type expense title" class="bg-gray-100 w-full p-2 border border-gray-300 rounded" value="">
      </div>
        <div class="mb-3">
        <label for="category"> Category</label>
        <select class="bg-gray-100 w-full p-2 border border-gray-300 rounded" name="category" id="category">
            <option value="" disabled selected>Select Expense Category</option>
            <option value="Staffing & Payroll">Staffing & Payroll</option>
            <option value="Utilities">Utilities</option>
            <option value="Maintenance">Maintenance</option>
            <option value="Housekeeping & Laundry">Housekeeping & Laundry</option>
             <option value="Food & Beverages">Food & Beverages</option>
             <option value="Guest Amenities">Guest Amenities</option>
             <option value="Marketing & Advertisement">Marketing & Advertisement</option>
              <option value="Licensing & Legal
">Licensing & Legal
</option>
  <option value="Technology">Technology
</option>
 <option value="Bank & Financial">Bank & Financial
</option>
        </select>
      </div>
     <div class="mb-3">
        <label for="amount" class="block text-gray-700">Amount</label>
         <input type="number" id="amount" step="0.01" name="amount" placeholder="Enter amount" class="bg-gray-100 w-full p-2 border border-gray-300 rounded" value="">
      </div>
       <div class="mb-3">
        <label for="expenseDate" class="block text-gray-700">Expense Date</label>
         <input type="date" id="expenseDate" name="expenseDate" class="bg-gray-100 w-full p-2 border border-gray-300 rounded" value="">
      </div>
        </div>
        <div class="mb-3">
        <label for="description" class="block text-gray-700">Description</label>
         <textarea id="description" name="description" placeholder="Type description of the expense" 
         rows="6" columns="4" class="bg-gray-100 w-full p-2 border border-gray-300 rounded" value=""></textarea>
      </div>
      <div class="mt-5">
        <button type="submit" name="submit" id="submitBtn" class="bg-gray-800 text-white px-6 py-2 rounded hover:bg-gray-700 hover:cursor-pointer">Add Expense</button>
      </div>
    </form>
  </div>
</main>

<!-- Scripts -->
<?php include_once 'includes/footer.php'; ?>
<script>
 

document.getElementById('addExpenseForm').addEventListener('submit', async function(e) {
  e.preventDefault();
  const submitBtn = document.getElementById('submitBtn');
  submitBtn.disabled = true;

  const formData = new FormData(addExpenseForm)
   console.log(formData)
  try {
    const response = await fetch('./functions/expense/add-expense.php', {
      method: 'POST',
      body: formData
    });

    let result;
    const contentType = response.headers.get("Content-Type");

    if (!response.ok) {
      // Try to extract the body if it’s still JSON error
      try {
        result = await response.json();
      } catch {
        throw new Error('Server error, invalid response');
      }
      throw new Error(result.errors ? result.errors.join('<br>') : 'Server error');
    }

    // Check if the response is JSON
    if (contentType && contentType.includes("application/json")) {
      result = await response.json();
    } else {
      throw new Error('Invalid response format');
    }

    if (result.success) {
      Swal.fire({
        icon: 'success',
        title: 'Expense Creation',
        text: 'Expense posted successfully!',
        timer: 2000,
        showConfirmButton: false
      });
      this.reset();
      window.location.href = "all-expenses.php";
    } else {
      Swal.fire({
        icon: 'error',
        title: 'Validation Error',
        html: (result.errors || ['Unknown error occurred.']).map(e => `<div>${e}</div>`).join('')
      });
      return;
    }

  } catch (err) {
    Swal.fire({
      icon: 'error',
      title: 'Error',
      html: err.message
    });
  } finally {
    submitBtn.disabled = false;
  }
});
</script>
</body>
</html>
